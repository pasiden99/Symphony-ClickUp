...
## Purpose
This page archives the former root `SPEC.md` for historical reference. It reflects an earlier, language-agnostic design that targeted Linear rather than the current ClickUp implementation.

## Covered Paths
- Former root `SPEC.md`

## Responsibilities
- Preserve the original specification text.
- Give maintainers historical context for how Symphony was initially framed before the current code settled on ClickUp-specific behavior.
- Keep legacy design material separate from the current implementation guides in `docs/`.

## Control and Data Flow
- Use this page for background only.
- Use [System Overview](../architecture/system-overview.md), [Domain Model](../architecture/domain-model.md), and the module pages for current runtime behavior.
- The archived specification remains below this note.

## Important Exports and Classes
- None. This page is historical prose, not a runtime API reference.

## Inputs and Outputs
- Input: the former root `SPEC.md` content.
- Output: an archived specification preserved inside the new documentation tree.

## Failure Modes
- This document diverges from the running implementation in important ways, including tracker choice, naming, and some architectural assumptions.
- Treating it as current behavior will lead to incorrect implementation decisions.

## Related Tests
- None directly. Current behavior is covered by the Vitest suites summarized in [Test Map](../testing/test-map.md).
